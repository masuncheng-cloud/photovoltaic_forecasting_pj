...
def export_season_days(df, dashboard_root):
    ...
    # 判断季节可用性：使用全部历史数据（包括 train），只要有真实功率数据就算可用
    # （train 行虽有 power_mw 但 power_pred_final 可能为空，所以先不过滤预测值）
    df_all_hist = build_history_frame(df)
    df_all_hist = df_all_hist[df_all_hist["power_mw"].notna()].copy()
