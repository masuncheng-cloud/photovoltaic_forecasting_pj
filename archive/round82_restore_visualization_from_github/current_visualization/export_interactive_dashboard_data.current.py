...
def export_season_days(df, dashboard_root):
    ...
    # 使用 build_history_frame（它已排除 future，保留 train/valid/test）
    df = df.copy()
    # power_pred_final only has data for valid+test; train rows are null.
    # Use power_pred_cal.fillna(power_pred_final) so all months have non-null pred.
    df["pred_mw"] = df["power_pred_cal"].fillna(df["power_pred_final"])
    df_f = build_history_frame(df)
    df_f = df_f[
        df_f["hour"].between(6, 19)
        & df_f["power_mw"].notna()
...
def export_season_best_days_by_site(df, dashboard_root):
    ...
    work = build_history_frame(df).copy()
    # build_history_frame copies pred_mw from df which may have nulls for valid+test.
    # Use the fallback that covers all splits: power_pred_cal.fillna(power_pred_final)
    if "actual_mw" not in work.columns:
        work["actual_mw"] = work["power_mw"]
    work["pred_mw"] = work["power_pred_cal"].fillna(work["power_pred_final"])
...
def export_season_best_days_city(df, dashboard_root):
    ...
    # build_history_frame already filters split and sets time/date/hour
    work = build_history_frame(df).copy()
    if "actual_mw" not in work.columns:
        work["actual_mw"] = work["power_mw"]
    # work may already have a stale pred_mw from main(); use the complete fallback
    work["pred_mw"] = work["power_pred_cal"].fillna(work["power_pred_final"])
    work["season"] = work["time"].dt.month.map(season_from_month)
    ...
    # Group by date (string) + hour — work.date is already a string from build_history_frame
    city = (
        work.groupby(["date", "hour", "season"], as_index=False)
        .agg(
            actual_mw=("actual_mw", "sum"),
            pred_mw=("pred_mw", "sum"),
            capacity_sum_mw=("capacity_mw", "sum"),
