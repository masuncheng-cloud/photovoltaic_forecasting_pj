...
def check_site(sid: str, json_rows: list, pkl_df: pd.DataFrame,
    ...
    # JSON 只导出 valid+test（不含 train），所以 n_pkl > n_json 是正常现象。
    # 正确的校验逻辑：n_matched == n_json（所有 JSON 行都能在 PKL 中匹配到），
    # 且匹配行的 actual/pred 误差为零。
    count_ok = (n_matched == n_json)
