...
def generate_report(data: dict, root: Path) -> str:
    ...
    lines.append("交互式预测结果页面位于 `stages/05_visualization/interactive_forecast_dashboard.html`。\n")
    lines.append("页面主要功能：")
    lines.append("- **站点筛选**：按站点ID、地区、误差等级（预测最好/最差/相对正确/样本少）筛选")
    lines.append("- **时间范围选择**：支持指定任意时间段查看历史预测效果")
    lines.append("- **散点图**：展示各站点样本量与 NRMSE 关系，按地区着色")
    lines.append("- **逐小时曲线**：展示城市总功率的真实值与预测值对比")
    lines.append("- **正午专题**：按日期分解正午时段城市总功率\n")
    lines.append("页面数据来源为 `output/pv_pipeline/tables/distributed_predictions_final_full.pkl`，")
    lines.append("仅用于展示当前 final/best 预测结果，不参与模型训练和模型选择。\n")
    ...
    lines.append("1. **部分站点 NRMSE 较高**：S019（首耀新海光伏）测试集 NRMSE 约 31%，显著高于整体水平，可能存在功率容量映射不准确的问题。\n")
    lines.append("2. **部分站点存在容量映射疑虑**：部分站点的实际发电功率峰值超过其登记装机容量，可能存在台账容量与实际不一致的情况，需人工核查数据字典。\n")
    lines.append("3. **正午时段误差仍然较高**：10-14时站点平均 NRMSE 在 13-15% 之间，高于早晚时段，主要原因在于高辐照度区间的功率曲线非线性特征更复杂。\n")
    lines.append("4. **完全复现训练结果需运行脚本**：如需在其他环境完全复现本结果，请运行 `scripts/train_fixed.py` 脚本，该脚本包含所有训练逻辑和随机种子设置。\n")
    lines.append("5. **部分站点历史数据不足**：部分 2025 年新并网站点（如 S037 华能中林三期光伏）仅有少量历史数据，预测可靠性相对较低。\n")
