...
def write_report(site_df, month_df, hour_df, dup_df, cap_df):
    ...
    if len(high_bad):
        lines.append(high_bad[cols].sort_values("test_nrmse_pct", ascending=False).to_markdown(index=False))
    else:
        lines.append("无")
    lines.append("")
    ...
    if len(low_good):
        lines.append(low_good[cols2].sort_values("test_nrmse_pct").head(15).to_markdown(index=False))
    else:
        lines.append("无")
    lines.append("")
