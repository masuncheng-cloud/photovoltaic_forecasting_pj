# Cursor 执行指令：生成阶段 2、3、4 训练效果图（Round97）

## 目标

基于当前完整项目代码和训练结果，在服务器项目目录：

```bash
/home/ac/data16t/msc/photovoltaic_forecasting_pj
```

生成 3 张用于 PPT 的“模型训练效果图”，不要与后面最终预测结果图重复：

1. 阶段 2：集中式功率反演辐照模型训练效果 4×4 红色系热力图  
2. 阶段 3：辐照融合权重 alpha 时段热力图  
3. 阶段 4：分布式功率模型特征重要性图  

输出目录统一为：

```bash
output/pv_pipeline/ppt_figures/model_effect/
```
...
required = [
    "output/pv_pipeline/metrics/inverse_metrics.csv",
    "output/pv_pipeline/metrics/irradiance_blend_metrics.csv",
    "output/pv_pipeline/metrics/distributed_metrics_v159.csv",
...
if missing:
    raise SystemExit("缺少必要输入文件，请先完整训练并导出结果。")
PY
```
...
```bash
scripts/generate_ppt_model_effect_figures_round97.py
```
...
python scripts/generate_ppt_model_effect_figures_round97.py

ls -lh output/pv_pipeline/ppt_figures/model_effect/
```
...
fig_dir = root / "output/pv_pipeline/ppt_figures/model_effect"

figs = [
    "阶段2_集中式反演辐照模型训练效果_4x4红色热力图.png",
    "阶段3_辐照融合权重alpha月小时热力图.png",
    "阶段4_分布式功率模型特征重要性图.png",
]
for f in figs:
    p = fig_dir / f
    print(("OK   " if p.exists() and p.stat().st_size > 50_000 else "BAD  "), f, p.stat().st_size if p.exists() else 0)
    if not p.exists() or p.stat().st_size <= 50_000:
        raise SystemExit(f"图文件异常：{f}")

stage2_csv = fig_dir / "阶段2_训练效果_4x4_nrmse数据.csv"
df = pd.read_csv(stage2_csv, index_col=0)
if df.shape != (4, 4):
    raise SystemExit(f"阶段2热力图不是4×4：{df.shape}")
if df.isna().any().any():
    raise SystemExit("阶段2热力图存在空值，不允许出现未导出。")

meta = json.loads((fig_dir / "model_effect_figures_metadata.json").read_text(encoding="utf-8"))
print("metadata round:", meta.get("round"))
print("stage4 importance source:", meta.get("stage4_importance_source"))
print("[PASS] Round97 PPT model effect figures generated.")
PY
```
...
```text
output/pv_pipeline/ppt_figures/model_effect/阶段2_集中式反演辐照模型训练效果_4x4红色热力图.png
output/pv_pipeline/ppt_figures/model_effect/阶段3_辐照融合权重alpha月小时热力图.png
output/pv_pipeline/ppt_figures/model_effect/阶段4_分布式功率模型特征重要性图.png
```

同时保留以下数据文件，方便核验：

```text
output/pv_pipeline/ppt_figures/model_effect/阶段2_训练效果_4x4_nrmse数据.csv
output/pv_pipeline/ppt_figures/model_effect/阶段3_融合权重alpha_月小时热力图数据.csv
output/pv_pipeline/ppt_figures/model_effect/阶段4_分布式功率模型特征重要性数据.csv
output/pv_pipeline/ppt_figures/model_effect/model_effect_figures_metadata.json
```
...
该图比较物理基线、反演辐照、IDW 空间扩展和融合辐照在训练集、验证集、测试集、整体上的 nRMSE。  
重点不是展示最终功率预测曲线，而是展示阶段 2 内部从物理估计到数据驱动修正的误差变化。

### 阶段 3：融合权重 alpha 热力图

alpha 越接近 1，说明模型更依赖 IDW 空间插值；alpha 越接近 0，说明模型更依赖 ERA5 气象背景。  
该图用于展示辐照融合权重随月份和小时变化，不是固定比例融合。

### 阶段 4：特征重要性图

该图展示分布式功率模型中贡献较高的变量。  
如果模型对象可以解析特征重要性，则使用模型原生 importance；若不可解析，则回退为训练表特征与容量归一化功率目标的绝对相关性，并在 metadata 中记录来源。
...
1. 不要覆盖最终预测结果文件。
2. 不要修改训练主流程。
3. 不要把这三张图接入可视化网页，它们只用于 PPT。
4. 若阶段 4 图使用了 fallback_abs_corr，需要在 PPT 中表述为“关键变量相关贡献”，不要表述为严格的模型特征重要性。
5. 阶段 2 热力图必须是 4×4，且不能出现“未导出”“无数据”等字样。
