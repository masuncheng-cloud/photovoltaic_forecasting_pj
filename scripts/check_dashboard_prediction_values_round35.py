...
def load_pkl() -> pd.DataFrame:
    ...
    print(f"  pkl: {len(df):,} 行 (不含future), {df['site_id'].nunique()} 站")
    return df
    ...
    for sid in sorted(json_data.keys()):
        ...
        if n_json == 0:
            rows.append({
                ...
                "max_abs_diff_actual": np.nan, "max_abs_diff_pred": np.nan,
                "max_abs_diff_capacity": np.nan, "status": "WARN",
        ...
        # 按 time 合并（仅对 capacity 产生 suffix，actual/pred/power_mw 无冲突则无 suffix）
        merged = pd.merge(
        ...
        if actual_ok and pred_ok and cap_ok:
            status = "PASS"
            msg = (f"actual_diff={max_diff_actual:.2e}, pred_diff={max_diff_pred:.2e}, "
                   f"cap_diff={max_diff_capacity:.2e}")
        else:
            all_pass = False
            status = "FAIL"
            msg = (f"actual_diff={max_diff_actual:.2e}, pred_diff={max_diff_pred:.2e}, "
...
def write_report(df: pd.DataFrame, all_pass: bool):
    ...
    lines = [
        ...
        f"**生成时间**: 2026-05-28 18:22\n",
        f"**检查文件**: distributed_predictions_final_round34.pkl vs interactive_dashboard/site_series/*.json\n",
        f"**容差**: |a - b| ≤ {TOLERANCE:.0e}\n",
        f"**默认排除**: split == 'future'\n",
        "\n## 总体结果\n",
        f"| 指标 | 值 |\n",
        f"|------|-----|\n",
    ...
    for _, r in df.iterrows():
        ...
        lines.append(
            ...
            f"| {ad} | {pd_} | {icon} {r['status']} | {r['message']} |\n"
        )
