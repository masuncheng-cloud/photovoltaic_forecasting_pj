#!/usr/bin/env python3
"""
evaluate_round72_candidate_on_test.py
================================
Round72 test 最终评估。
...
def compute_city_nrmse_by_hours(df, pred_col, hours):
    vals = []
...
def compute_city_bias_by_hours(df, pred_col, hours):
    d = df[df["hour"].isin(hours)]
...
def main():
    parser = argparse.ArgumentParser(description="Round72 Test 评估")
    ...
    # 读取诊断和决策
    decision_path = OUT / "round72_candidate_decision.json"
    decision = {}
    if decision_path.exists():
        with open(decision_path) as f:
            decision = json.load(f)

    diag_path = OUT / "round72_prediction_column_audit_summary.json"
    audit = {}
    if diag_path.exists():
        with open(diag_path) as f:
            audit = json.load(f)

    candidate_cols = [c for c in df.columns if c.startswith("power_pred_round72_")]
    ...
    for col in [bl_col] + candidate_cols:
        ...
        overall_rows.append({
            ...
            "city_nrmse_10_14": round(compute_city_nrmse_by_hours(test_df, col, focus_hours), 4),
            "site_mean_nrmse_6_19": round(compute_site_mean_nrmse(test_df, col), 4),
            "city_bias_6_19": round(compute_city_bias(test_df, col), 4),
            "city_bias_10_14": round(compute_city_bias_by_hours(test_df, col, focus_hours), 4),
            "rmse_city": round(rmse(
                test_df.groupby("time")["power_mw"].sum().values,
                test_df.groupby("time")[col].sum().values), 4),
        })
    overall_df = pd.DataFrame(overall_rows)
    overall_df.to_csv(OUT / "round72_test_overall_compare.csv",
                     index=False, encoding="utf-8-sig")
    print(overall_df.to_string(index=False))
    print(f"[OK] {OUT / 'round72_test_overall_compare.csv'}")
    ...
    hourly_df.to_csv(OUT / "round72_test_hourly_compare.csv",
                     index=False, encoding="utf-8-sig")
    print(f"[OK] {OUT / 'round72_test_hourly_compare.csv'}")
    ...
    site_df.to_csv(OUT / "round72_test_site_compare.csv",
                   index=False, encoding="utf-8-sig")
    print(f"[OK] {OUT / 'round72_test_site_compare.csv'}")
    ...
    # ── 报告 ──────────────────────────────────────────────────────────
    print("\n[Generating Report]")
    report_path = PROJECT_ROOT / "docs" / "Round72_重建全历史一致基线并重新训练残差模型报告.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)

    baseline_test = overall_df[overall_df["candidate"] == bl_col].iloc[0]

    report = f"""# Round72 重建全历史一致基线并重新训练残差模型报告
...
**power_pred_final 在 train 缺失**: {audit.get('key_findings', {}).get('power_pred_final_missing_in_train', 'N/A')}

**一致可用列**: {audit.get('key_findings', {}).get('train_valid_test_consistent_columns', [])}
...
"""
    for _, row in overall_df.iterrows():
        ...
        report += f"| {row['candidate']} | {row['city_nrmse_6_19']}% | {delta_n:+.3f}pp | {row['site_mean_nrmse_6_19']}% | {row['city_bias_6_19']}% | {row['city_bias_10_14']}% |\n"
