...
if (TABLES / "site_master.csv").exists():
    sm = pd.read_csv(TABLES / "site_master.csv")
    # 注意：site_master 用的是 lat/lon，不是 latitude/longitude
...
if train_path.exists():
    ...
    # 真正泄漏的是：不应该作为特征的列（排除 split 标签和目标变量本身）
    # power_mw 是训练目标（y=power_mw/capacity），split 是后验标签，不算特征泄漏
    # 但预测阶段不应出现的列才算泄漏
